...
contract con_falcon_signature_verify
{
   ...
   // At first, an empty "address"-type variable of the name "creator". Will be set in the constructor.
    ...
    function constuctor(string memory _algorithm) public
    {
    ...
    function setAlgorithm(string memory _newAlgorithm) public
    {
    ...
    {
        if (msg.sender == m_creator)   // Only allow this action if the account sending the signal is the creator
        {
            selfdestruct(m_creator);   // kills this contract and sends remaining funds back to creator
        }
    }
